...
{
    class BlurPopup_Bilder : IPopupBuilder
    {
        public override IPopup GetResult()
        {
            ...
            BlurPopup result = new BlurPopup
            {
                ...
                widthValue_Text = new Label() { Text = "ksize width:   " },
                HeightValue_Text = new Label() { Text = "ksize height:   " },
                xposValue_Text = new Label() { Text = "anchor xpos:      < 0 ; ksize width ]" },
                yposValue_Text = new Label() { Text = "anchor ypos:      < 0 ; ksize height ]" },
                // anchor.x  >=  0 
                // anchor.x  <  k.Width
                // anchor.y  >=  0
                // anchor.Y  <  k.Height
    ...
    class BlurPopup : IPopup
    {
        ...
        public void Value_ValueChanged(object sender, EventArgs e)
        {
            ...
            if (xposValue.Value < 0)                        // anchor.x  >=  0 
            {
                ++test;
                xposValue_Value.ForeColor = Color.Red;
            }
            else if (xposValue.Value >= widthValue.Value)   // anchor.x  <  k.Width
            {
                ++test;
                xposValue_Value.ForeColor = Color.Red;
            }
            ...
            {
                xposValue_Value.ForeColor = Color.Black;
            }
            
            
            if (yposValue.Value < 0)                        // anchor.y  >=  0
            {
                ++test;
                yposValue_Value.ForeColor = Color.Red;
            }
            else if (yposValue.Value >= HeightValue.Value)  // anchor.Y  <  k.Height
            {
                ++test;
                yposValue_Value.ForeColor = Color.Red;
            }
            ...
            {
                yposValue_Value.ForeColor = Color.Black;
            }
